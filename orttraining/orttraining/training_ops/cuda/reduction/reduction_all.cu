...
void ScalarSqrt(cudaStream_t stream, Tin* input, Tout* output) {
  _ScalarSqrtImpl<<<1, 1, 0, stream>>>(input, output);
};
...
void MultiTensorReduce(cudaStream_t stream, ChunkGroup<1> chunk_group, TOut* output) {
  ...
  _MultiTensorReduceImpl<TIn, TOut, TBuf, TInOp, TOutOp><<<chunk_group.chunk_count, thread_count, shared_memory_size, stream>>>(chunk_group, output);
}
