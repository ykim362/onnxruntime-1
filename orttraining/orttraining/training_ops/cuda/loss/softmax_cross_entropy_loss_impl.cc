...
Status SoftmaxCrossEntropyLoss<T, Tin>::ComputeInternal(OpKernelContext* ctx) const {
  ...
  ORT_ENFORCE(cudaMemsetAsync(weight_data_nd_data, 0, N_D * sizeof(T), Stream()) == cudaSuccess);
  ...
  if (reduction_ == ReductionType::MEAN) {
    ...
    cudaMemcpyAsync(normalize_factor_data.get(), &normalize_factor, sizeof(T), cudaMemcpyHostToDevice, Stream());
  }
  ...
  if (logit_shape.NumDimensions() > 2 && log_prob != nullptr) {
    ...
    cudaMemcpyAsync(log_prob_data, transposed_data, sizeof(T) * logit_shape.Size(), cudaMemcpyDeviceToDevice, Stream());
...
Status SoftmaxCrossEntropyLossGrad<T, Tin>::ComputeInternal(OpKernelContext* ctx) const {
  ...
  if (probability_shape.NumDimensions() > 2) {
    ...
    cudaMemcpyAsync(d_logit_data, transposed_data, sizeof(T) * probability_shape.Size(), cudaMemcpyDeviceToDevice, Stream());
