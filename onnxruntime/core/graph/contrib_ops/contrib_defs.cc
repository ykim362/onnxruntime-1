...
attends to its W/2 previous tokens and W/2 succeding tokens with W being the window length. A selected few tokens
attend globally to all other tokens.

The attention_mask is of shape (batch_size, sequence_length), where sequence_length is a multiple of 2W after padding.
Mask value < 0 means the token is masked, 0 means local attention, and >0 global attention.
)DOC";
    ...
    ONNX_CONTRIB_OPERATOR_SCHEMA(LongformerAttention)
        ...
        .Attr("num_heads", "Number of attention heads", AttributeProto::INT)
        .Attr("window", "Attention windows length W", AttributeProto::INT)
        .Input(0, "input", "3D input tensor with shape (batch_size, sequence_length, hidden_size), hidden_size = num_heads * head_size", "T")
        .Input(1, "weight", "2D input tensor with shape (hidden_size, 3 * hidden_size)", "T")
        .Input(2, "bias", "1D input tensor with shape (3 * hidden_size)", "T")
        .Input(3, "mask", "Attention mask with shape (batch_size, sequence_length)", "M")
        .Output(0, "output", "3D output tensor with shape (batch_size, append_length, hidden_size)", "T")
        .TypeConstraint("T", {"tensor(float)", "tensor(float16)"}, "Constrain input and output types to float tensors.")
