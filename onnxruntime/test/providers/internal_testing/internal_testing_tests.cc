...
TEST(InternalTestingEP, Basic) {
  std::string model_path("testdata/tmp/sauvola_classification4_epoch-88.onnx");

  SessionOptions so;
  InferenceSessionWrapper session(so, GetEnvironment());

  // set supported ops to Conv, Relu and MaxPool as these form a chain through the model.
  // we can say the EP potentially handles them all, but can also simulate removing handling of MaxPool
  ...
  //
  // NOTE: We need a kernel hash for the CPU versions of all nodes in the EP that may drop down
  //
  const std::vector<std::string> supported_ops{"Conv", "Relu", "MaxPool"};
  ...
  for (const auto& node : graph.Nodes()) {
    std::cout << node.GetExecutionProviderType() << "\n";
  }
}
...
TEST(NnapiExecutionProviderTest, FunctionTest) {
  ...
  status = session.RegisterExecutionProvider(onnxruntime::make_unique<::onnxruntime::NnapiExecutionProvider>());
  ASSERT_TRUE(status.IsOK());
  ...
  // Now run
  status = session.Run(run_options, feeds, output_names, &fetches);
  ASSERT_TRUE(status.IsOK());
